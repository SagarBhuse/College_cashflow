<?php
//error_reporting(0);
$_GET['transtype'] = "Expense";
include_once("databaseconnection.php");
if(isset($_POST['esubmit']))
{
	if(isset($_POST['transeditid']))
	{
		$sql ="UPDATE transaction SET transtype='Expense',billingtype='General',billno='0',totalamt='$_POST[icost]',transdate='$_POST[itrandate]',accountid='$_POST[iaccountid]',transdetails='$_POST[idescr]',paymentdetail='$_POST[ipaytype]',status='Active',categoryid='$_POST[icategoryid]' WHERE transactionid='$_POST[transeditid]'";
		$qsql = mysqli_query($con,$sql);
		
		$sql ="UPDATE incomeexpenserec SET accountid='$_POST[iaccountid]', transtype='Expense', serviceid='0', qty='1', cost='$_POST[icost]', trandate='$_POST[itrandate]', status='Active', title='$_POST[ititle]' WHERE transactionid='$_POST[transeditid]'";
		$qsql = mysqli_query($con,$sql);
	...
	{
		$sql ="INSERT INTO transaction(transtype,billingtype,billno,totalamt,transdate,accountid,transdetails,paymentdetail,status,categoryid) values('Expense','General','0','$_POST[icost]','$_POST[itrandate]','$_POST[iaccountid]','$_POST[idescr]','$_POST[ipaytype]','Active','$_POST[icategoryid]')";
		$qsql = mysqli_query($con,$sql);
		$insid = mysqli_insert_id($con);
		
		$sql ="INSERT INTO incomeexpenserec(transactionid, accountid, transtype, serviceid, qty, cost, trandate, status, title) values('$insid','$_POST[iaccountid]','Expense','0','1','$_POST[icost]','$_POST[itrandate]','Active','$_POST[ititle]')";
		$qsql = mysqli_query($con,$sql);
		if(mysqli_affected_rows($con) ==1)
		{
			echo "<script>alert('Expense Entry done successfully...');</script>";
			echo "<script>window.location='viewietransaction.php?accountid=$_POST[iaccountid]';</script>";
		}
		else
		{
...
<div class="row">
	<div class="col-md-4"></div>
	<div class="col-md-5">
		<input class="form-control"  type="submit" name="esubmit" id="esubmit" value="Submit">
	</div>
	<div class="col-md-4"></div>
</div>
</form>
