...
<form method="post" action="">
      ...
      <div class="card">
        <div class="card-header">
          <h3 class="card-title">View Transaction Details - <b><?php 
...
<table id="myTable"  class="table table-striped table-bordered" >
	<thead>
		<tr>
			...
			<th style="width: 205px;">Transaction Detail</th>
<?php 
		  if($_GET['transtype'] == "Income")
		  {
 ...
 ?>			
	...
	$totincome = 0;
	$totexpense = 0;
	$sql ="SELECT * FROM transaction WHERE transaction.status='Active'";
	if($_GET['transtype'] == "Income")
	{
	...
	while($rs = mysqli_fetch_array($qsql))
	{
		...
		echo "<td style='text-align: right;' ><a href='' onclick='loadtransdetail($rs[0],`$rs[transtype]`,`$actype`)'  class='btn btn-dark'  data-toggle='modal' data-target='#myreportModal'>View</a></td>				
			</tr>";
	}
	 ?>
	</tbody>
	<tfoot>
		...
		<?php
		  if($_GET['transtype'] == "Income")
		  {
			  echo "<th style='width: 65px;'>₹$totincome</th>";
		  }
		  else if($_GET['transtype'] == "Expense")
		  {
			  echo "<th style='width: 65px;'>₹$totexpense</th>";
		  }
		  else
		  {
...
if(transtype == "Income")
{
document.getElementById("divtransbuttons").innerHTML = "<a class='btn btn-danger' style='color: white;' href='viewietransaction.php?deltransactionid=" + transid + "' onclick='return confirmtodelete()' >Delete</a><a target='_blank'  class='btn btn-warning' href='ajaxtransdet.php?transid=" + transid + "&action=print' >Print</a>";
}
